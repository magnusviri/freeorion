        ...
        if (!use_binary) {
            if (use_zlib_for_zml) {
                ...
                try {
                    ...
                    //using InsertDevice = boost::iostreams::back_insert_device<std::string>;
                    //InsertDevice serial_inserter(compressed_str);
                    //boost::iostreams::stream<InsertDevice> c_sink(serial_inserter);

                    // set up compression-filter
                    std::ostringstream compressed_os;
                    boost::iostreams::filtering_ostream zos;
                    //boost::iostreams::filtering_ostreambuf zos;
                    zos.push(boost::iostreams::zlib_compressor(boost::iostreams::zlib_params(
                        boost::iostreams::zlib::default_compression)));
                    zos.push(boost::iostreams::base64_encoder());
                    zos.push(compressed_os);
                    //zos.push(c_sink);
                    //std::ostream os(&zos);
                    ...
                    timer.EnterSection("universe to xml");
                    {
                        ...
                        freeorion_xml_oarchive xoa(zos);
                    ...
                    if (!zos.strict_sync())
                        zos.reset();
                    //DebugLogger() << "compressed string sz: " << compressed_str.size() << " :" << compressed_str;
                    ...
                    timer.EnterSection("universe to xml");
                    xoa2 << boost::serialization::make_nvp("compressed_str", compressed_os.str());
